...
<xsl:stylesheet 
    ...
    <!-- Folha de estilo atualizada para extrair dados de orbita de sol_2010 
    ...
    <xsl:variable name="itensMostrados" select="$objetosEscolhidos[@diametrokm &gt; $minDiametro or self::cometa][@diametrokm &lt;= $maxDiametro][ancestor-or-self::node()/@afelioUA &gt;= $minOrbita or self::estrela][ancestor-or-self::node()/@afelioUA &lt;= $maxOrbita or self::estrela or self::estrela]" />
                ...
                <g id="titulo">
                    ...
                    <xsl:choose>
                        <xsl:when test="not($subtitulo)">
                            <text y="50" font-size="12pt">Objetos com afelio entre <xsl:value-of select="format-number($minOrbita,'###.##0,##')"/> e <xsl:value-of select="format-number($maxOrbita,'###.##0,##')"/> UA</text>
                        </xsl:when>
                        <xsl:otherwise>
    ...
    <xsl:template match="sistemaSolar">
        <xsl:for-each select="$itensMostrados">
            <xsl:sort select="@afelioUA" order="descending" data-type="number"/>
            <g>
                ...
                <rect x="{$larguraNome + $maisX}" y="{(position() - 1) * $alturaItem}" width="{(@afelioUA div $maxOrbita)*$maiorBarra * $escalaHorizontal}" height="{$alturaBarra}">
                ...
                <text font-size="12pt" x="{(@afelioUA div $maxOrbita)*$maiorBarra*$escalaHorizontal + $larguraNome + 10 + $maisX}" y="{$alturaItem div 2 + (position() - 1)*$alturaItem}"><xsl:value-of select="format-number(@afelioUA,'###.##0,##')"/> UA</text>
            </g>
        </xsl:for-each>
    </xsl:template>
